...
char *read_line() 
{
	...
	while (1) {
		...
		if (ch >= 32 && ch != 127) { // It is a printable character. 
			...
			unsigned char append_history = 1;
			line_buffer[line_length] = 0;
			if(line_buffer[0] != 0) {
				if(history_length > 0) {
					if(strcmp(history[history_length-1], line_buffer) == 0) { // Do not append to history if the same
						append_history = 0;
					}
				}
				if(append_history != 0) {
			...
			if(ch1 == 91 && ch2 == 66) {
				...
				//if(history_index > 0) { // Copy line from history
				if(history_index < (history_length - 1)) { // Copy line from history
					strcpy(line_buffer, history[history_index]);
					line_length = strlen(line_buffer);
					//history_index=(history_index-1)%history_length;
					history_index++;
